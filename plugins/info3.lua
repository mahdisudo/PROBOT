do
local MKH = 105616381 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = 'ندارد'
    ...
    local text = 'نام کامل : <code>'..(result.first_name or '')..' '..(result.last_name or '')..'</code>\n'
               ..'یوزر: '..Username..'\n'
  ...
  text = text..'تعداد پیام های فرستاده شده : <code>'..user_info_msgs..'</code>\n لینک کاربر = http://telegram.me/'..msg.from.username..'\n\n'
  	 local hashs = 'chmsgs:'..msg.to.id
        local msgss = (redis:get(hashs) or '0')
    local pm = math.floor(user_info_msgs / msgss * 100)
  text = text..'<code>پیام های شما به % : <code>'..pm..'</code>\nکل پیام ها :'..msgss..'\n\n</code>'
  text = text..''
 ...
 if result.username then
  ...
  text = text..'تعداد پیام های فرستاده شده : <code>'..user_info_msgs..'</code>\n لینک کاربر = http://telegram.me/'..msg.from.username..'\n\n'
  	 local hashs = 'chmsgs:'..msg.to.id
        local msgss = (redis:get(hashs) or '0')
    local pm = math.floor(user_info_msgs / msgss * 100)
  text = text..'پیام های شما به% : <code>'..pm..'</code>\nکل پیام ها :<code>'..msgss..'</code>\n\n'
  text = text..''
  send_msg(extra.receiver, text, ok_cb,  true)
  else
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = 'نام : <code>'..(result.from.first_name or '')..'\nفامیل :'..(result.from.last_name or '----')..'</code>\n'
               ..'یوزرنیم : '..Username..'\n'
		...
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.peer_id == tonumber(MKH) then
		       text = text..'مقام : مدیر کل ربات <code>Executive Admin</code> \n\n'
		     elseif is_admin2(result.from.peer_id) then
		       text = text..'مقام : ادمین <code>Admin</code> \n\n'
		     elseif is_owner2(result.from.peer_id, result.to.peer_id) then
		       text = text..'مقام : مدیر کل گروه <code>Owner</code> \n\n'
		     elseif is_momod2(result.from.peer_id, result.to.peer_id) then
		       text = text..'مقام : مدیر گروه <code>Moderator</code> \n\n'
		 else
		       text = text..'مقام : کاربر <code>Member</code> \n\n'
			end
		  else
		   text = text..'مقام : '..value..'\n\n'
		 end
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = 'نام : <code>'..(msg.from.first_name or 'ندارد')..'</code>\n'
	...
	if hash then
	  ...
	  if not value then
		...
		elseif is_sudo(msg) then
		 text = text..'مقام : ادمین ربات <code>(Admin)</code> \n\n'
